document.addEventListener('DOMContentLoaded', async () => {
  ...
  RQBOX.id('guide-btn-power')?.addEventListener('click', () => {
    RQBOX.openModal(`
      <h3>🚪 Exit RQBBOX Experience OS 1.0?</h3>
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Meta' || e.key === 'OS') {
      if (document.fullscreenElement) {
        RQBOX.toast('Press Win+F11 to return to RQBBOX Experience OS 1.0');
      }
    }
  });
  ...
  setTimeout(() => {
    if (GameLibrary.games.length === 0) {
      RQBOX.toast('👋 Welcome to RQBBOX Experience OS 1.0! Navigate with controller or keyboard.');
    }
